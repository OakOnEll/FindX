...
public class DelayedTextView extends TextView {
	...
	public void writeText(Spanned text) {
	...
	public void writeWithDelay(final SoundInfo delayInfo,
			final TextViewInfo info) {
		if (pendingRunnable != null) {
		...
		pendingRunnable = new Runnable() {
			...
			public void run() {
				iter.count++;
				Spanned fromHtml = Html.fromHtml(info.text);
				if (iter.count <= fromHtml.length()) {
					setText(fromHtml.subSequence(0, iter.count));
