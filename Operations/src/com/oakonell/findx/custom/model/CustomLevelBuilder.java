...
public class CustomLevelBuilder {
    ...
    public void setSolution(Fraction solution) {
        ...
        Equation solvedEquation = new Equation(new Expression(1, 0), new Expression(Fraction.ZERO, solution));
        ...
        List<Move> oldMoves = new ArrayList<Move>(moves.subList(1, moves.size()));
    ...
    public void replaceMoves(List<Move> result) {
        ...
        if (!newLastMove.getEndEquation().equals(originalLastMove.getEndEquation())) {
            throw new RuntimeException("The end equations should match upon replacing the moves!");
        }
    }
    ...
    public void apply(Operation op) {
        ...
        if (op == null || !operations.contains(op)) {
            throw new IllegalArgumentException("Operation " + op + " is not one of the level's valid operations");
        }
    ...
    public void defaultMaxSequence() {
        ...
        int max = 0;
        Cursor cursor = db.rawQuery("select MAX(" + DataBaseHelper.CustomLevelTable.SEQ_NUM + ") as max_seq from "
    ...
    public void moveUp(Move item) {
        ...
        List<Move> toReapply = new ArrayList<Move>(subList.subList(0, subList.size() - 1));
        subList.clear();
        moves.remove(0);
    ...
    public void moveDown(Move item) {
        ...
        List<Move> toReapply = new ArrayList<Move>(subList.subList(0, subList.size()));
    ...
    public List<Integer> getMoveOperationIndices() {
        ...
        for (Move each : getMoves()) {
            ...
            if (indexOf == -1) {
                throw new RuntimeException("The custom level " + id + " solution moves contains an invalid operation "
                        + each.getOperation());
            }
            result.add(indexOf);
        }
