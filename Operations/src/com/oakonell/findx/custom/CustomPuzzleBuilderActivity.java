...
public class CustomPuzzleBuilderActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new ArrayAdapter<Move>(getApplication(),
            ...
            public View getView(int position, View row, ViewGroup parent) {
                if (row == null) {
                    row = getLayoutInflater().inflate(R.layout.move_build, parent, false);
                }
                ...
                if (position > 0) {
                    ...
                    op.setOnClickListener(new OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            PopupContextMenu menuDialog = new PopupContextMenu(CustomPuzzleBuilderActivity.this, 2);
                            menuDialog.addItem(getResources(), R.string.edit_move, 2);
                            menuDialog.addItem(getResources(), R.string.delete_move, 3);
                            ...
                            Dialog menu = menuDialog.createMenu(getText(R.string.move_menu_title));
                            menu.show();
                        }
                    });
                } else {
        ...
        addOperator = (Button) findViewById(R.id.add_op);
        ...
        builder.setOptimizedListener(new OptimizedListener() {
            ...
            public void isOptimized(boolean optimized) {
                minimizeMoves.setEnabled(!optimized && !builder.getOperations().isEmpty());
            }
        });
    ...
    protected void promptToOptimizeOrSave() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton(R.string.optimize, new DialogInterface.OnClickListener() {
                    @Override
    ...
    private void handleOperatorButton(int op1, List<Operation> operations, int i) {
        final Button opButton = (Button) findViewById(op1);
        ...
        opButton.setText(operation.toString());
    ...
    private boolean presentOpButtonChoices(final Button opButton, final Operation operation) {
        ...
        Dialog menu = menuDialog.createMenu(getText(R.string.operation_menu_title));
        menu.show();
        return false;
    }

    private void deleteOperation(Button opButton, final Operation operation) {
        if (builder.usesOperation(operation)) {
            ...
            alertDialog
                    .setMessage(getResources().getString(R.string.confirm_delete_operation_is_used,
                            operation.toString()));
    ...
    private void editOperation(final Operation operation) {
        OperationBuilderDialog expressionBuilder = new OperationBuilderDialog(randomHelper);
        expressionBuilder.buildExpression(this, operation, new OperationBuiltContinuation() {
            @Override
    ...
    private void addAnOperator() {
        OperationBuilderDialog expressionBuilder = new OperationBuilderDialog(randomHelper);
        expressionBuilder.buildExpression(this, null, new OperationBuiltContinuation() {
            @Override
    ...
    private void confirmAndLeave() {
        ...
        alertDialog
                .setMessage(getResources().getString(R.string.confirm_lose_level_changes));
    ...
    private void deleteMove(final Move item) {
        ...
        alertDialog.setMessage(getResources().getString(R.string.confirm_delete_move));
        alertDialog.setIcon(android.R.drawable.ic_delete);
        alertDialog.setButton(getText(android.R.string.ok), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
    ...
    private void editMove(final Move item) {
        final CharSequence[] items = new CharSequence[builder.getOperations().size()];
        int i = 0;
        for (Operation each : builder.getOperations()) {
            items[i] = each.toString();
            i++;
        }
        final int defaultItemIndex = builder.getOperations().indexOf(item.getOperation());

        AlertDialog.Builder dbuilder = new AlertDialog.Builder(this);
        dbuilder.setTitle(R.string.choose_new_operation);
        dbuilder.setSingleChoiceItems(items, defaultItemIndex, new DialogInterface.OnClickListener() {
            @Override
