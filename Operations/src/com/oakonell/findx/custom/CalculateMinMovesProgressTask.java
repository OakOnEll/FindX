...
class CalculateMinMovesProgressTask extends
		AsyncTask<Void, SolverState, Solution> {
	...
	public CalculateMinMovesProgressTask(CustomPuzzleBuilderActivity context,
	...
	private void showDialog() {
		...
		Button cancelButton = (Button) dialogInfo.dialog
		...
		dialogInfo.progressText = (TextView) dialogInfo.dialog
				.findViewById(R.id.value);
		dialogInfo.maxText = (TextView) dialogInfo.dialog
				.findViewById(R.id.max);
		dialogInfo.progress = (ProgressBar) dialogInfo.dialog
				.findViewById(R.id.progress);
		dialogInfo.solutionFound = (TextView) dialogInfo.dialog
				.findViewById(R.id.found_solution);
	...
	protected Solution doInBackground(Void... params) {
		...
		OnCalculateMove onCalculateMove = new OnCalculateMove() {
			...
			public void calculated(SolverState currentState) {
				...
				if (lastNumUpdated == 0
						|| currentState.getNumMovesVisited() - lastNumUpdated > 100) {
	...
	protected void onPostExecute(final Solution result) {
		if (result == null || result.getNumMoves() >= builder.getNumMoves()) {
			...
			alertDialog.setMessage(parent.getResources().getString(
					R.string.already_minimum_moves, builder.getNumMoves()));
			alertDialog.setButton(parent.getText(android.R.string.ok),
	...
	private void promptToReplaceMoves(final Solution result,
			final boolean optimal) {
		final AlertDialog alertDialog = new AlertDialog.Builder(parent)
				.create();
		alertDialog.setTitle(R.string.better_solution_title);
		alertDialog.setMessage(parent.getResources().getString(
				R.string.better_solution, result.getNumMoves()));
		alertDialog.setButton(parent.getText(android.R.string.ok),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface d, int which) {
		...
		alertDialog.setButton2(parent.getText(android.R.string.cancel),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface d, int which) {
						alertDialog.dismiss();
	...
	protected void onProgressUpdate(SolverState... values) {
		...
		// TODO
		if (currentSolutionDepth > 0
				&& currentSolutionDepth < lastSolutionDepth) {
			lastSolutionDepth = currentSolutionDepth;
			dialogInfo.solutionFound.setVisibility(View.VISIBLE);
			dialogInfo.solutionFound.setText(parent.getResources().getString(
					R.string.found_solution, currentSolutionDepth));
		}
	}
