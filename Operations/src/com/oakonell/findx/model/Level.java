...
public class Level {
    ...
    public Level(Stage stage, String name, Equation equation, List<Operation> operations,
            List<Integer> solutionOpIndices) {
        this.stage = stage;
    ...
    private int getExistingRating(SQLiteDatabase db) {
        ...
        if (query.moveToFirst()) {
            rating = 3;
            int movesIndex = query.getColumnIndex(DataBaseHelper.LevelProgressTable.MIN_MOVES);
            int numMoves = query.getInt(movesIndex);

            int numUndosIndex = query.getColumnIndex(DataBaseHelper.LevelProgressTable.NUM_UNDOS);
    ...
    private Cursor getExistingRatingRow(SQLiteDatabase db) {
        Cursor query = db.query(DataBaseHelper.LEVEL_PROGRESS_TABLE_NAME, null,
                DataBaseHelper.LevelProgressTable.LEVEL_ID + "=?", new String[] { id }, null, null, null);
        return query;
    }
    ...
    public void possibilyUpdateRating(int moves, int undosUsed) {
        ...
        if (existingRating > 0) {
            // update
            db.update(DataBaseHelper.LEVEL_PROGRESS_TABLE_NAME, values, DataBaseHelper.LevelProgressTable.LEVEL_ID
                    + "=?", new String[] { id });
        } else {
            // insert
    ...
    public String getMultilineDescription() {
        ...
        for (Iterator<Operation> iter = getOperations().iterator(); iter.hasNext();) {
