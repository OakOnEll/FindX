...
public class Puzzle {
    ...
    public void initialize(String puzzleId, List<Integer> opIndices, int numUndosUsed) {
        level = Levels.get(puzzleId);
        moves.clear();
        ...
        for (int index : opIndices) {
            if (index >= operations.size()) {
                throw new IllegalArgumentException("Operator index out of bounds: " + index);
            }
    ...
    public void writeState(SQLiteDatabase db) {
        ...
        levelInfo.put(DataBaseHelper.CurrentLevelTable.NUM_UNDOS_USED, getUndosUsed());
        db.insert(DataBaseHelper.CURRENT_LEVEL_STATE_TABLE_NAME, null, levelInfo);

        int seq = 1;
        for (Move each : moves) {
            ContentValues moveInfo = new ContentValues();
            moveInfo.put(DataBaseHelper.CurrentLevelMovesTable.LEVEL_ID, getId());
            moveInfo.put(DataBaseHelper.CurrentLevelMovesTable.SEQUENCE, seq);
            int opIndex = getOperatorIndex(each);
            moveInfo.put(DataBaseHelper.CurrentLevelMovesTable.OP_INDEX, opIndex);
            db.insert(DataBaseHelper.CURRENT_LEVEL_MOVES_TABLE_NAME, null, moveInfo);
            seq++;
        }
    }
    ...
    public static String readPendingLevel(SQLiteDatabase db) {
        try {
            Cursor query = db.query(DataBaseHelper.CURRENT_LEVEL_STATE_TABLE_NAME, null, null, null, null, null, null);
            if (!query.moveToFirst()) {
                query.close();
                return null;
            }

            int idIndex = query.getColumnIndex(DataBaseHelper.CurrentLevelTable.LEVEL_ID);
    ...
    public static void readState(SQLiteDatabase db, String inLevelId, Puzzle puzzle) {
        try {
            Cursor query = db.query(DataBaseHelper.CURRENT_LEVEL_STATE_TABLE_NAME, null, null, null, null, null, null);
            if (!query.moveToFirst()) {
                query.close();
            ...
            int numUndosIndex = query.getColumnIndex(DataBaseHelper.CurrentLevelTable.NUM_UNDOS_USED);
            int numUndosUsed = query.getInt(numUndosIndex);
            query.close();

            List<Integer> opIndices = new ArrayList<Integer>();
            Cursor moveQuery = db.query(DataBaseHelper.CURRENT_LEVEL_MOVES_TABLE_NAME, null, null, null, null, null,
            ...
            while (moveQuery.moveToNext()) {
                ...
                int opIndexIndex = moveQuery.getColumnIndex(DataBaseHelper.CurrentLevelMovesTable.OP_INDEX);
                int opIndex = moveQuery.getInt(opIndexIndex);
                if (opIndex < 0) {
                    throw new IllegalArgumentException("Operator index out of bounds: " + opIndex);
                }
    ...
    public void apply(Operation operation) {
        if (!getOperations().contains(operation)) {
            throw new RuntimeException("Operation " + operation + " is not a valid operation for puzzle " + getId());
        }
