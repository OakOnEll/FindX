 ...
 */
public class CustomKeyboard implements
	...
	private OnKeyboardActionListener mOnKeyboardActionListener = new OnKeyboardActionListener() {
		...
		public final static int CodeConv = 55011; // Conversions like round or
													// degrees
		public final static int CodeTrig = 55012;

		@Override
		public void onKey(int primaryCode, int[] keyCodes) {
			Log.i("CustomKeyboard", "onKeyBoardAction ");
	...
	public CustomKeyboard(Activity host, int viewid, int layoutid) {
		this(host, null, (KeyboardView) host.findViewById(viewid), layoutid);
	}

	public CustomKeyboard(Activity host, Dialog dialog, KeyboardView view,
			int layoutid) {
		mHostActivity = host;
		mKeyboardView = view;
		...
		// set the window to the activity's or the dialog's window
		if (dialog != null) {
			window = dialog.getWindow();
		} else {
			window = mHostActivity.getWindow();
		}
	...
	public void registerEditText(final EditText edittext) {
		...
		edittext.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				Log.i("CustomKeyboard", "edittext.onFocusChange " + v.getId());
				if (hasFocus)
		...
		edittext.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Log.i("CustomKeyboard", "edittext.onClick " + v.getId());
				showCustomKeyboard(v);
			}
		});
		 ...
		 * 
		 * @source 
		 *         http://androidpadanam.wordpress.com/2013/05/29/customkeyboard-
		 *         example/ fixes the cursor not movable bug
		 */
		OnTouchListener otl = new OnTouchListener() {

			@Override
			public boolean onTouch(View v, MotionEvent event) {
				Log.i("CustomKeyboard", "edittext.onTouch " + v.getId());
				if (!v.hasFocus()) {
					v.requestFocus();
				}
				...
				case MotionEvent.ACTION_DOWN:
					...
					if (offset > 0)
						if (x > layout.getLineMax(0))
							edittext.setSelection(offset); // touch was at the
															// end of the text
						else
				...
				case MotionEvent.ACTION_MOVE:
					...
					if (offset > 0)
						if (x > layout.getLineMax(0))
							edittext.setSelection(offset); // Touchpoint was at
															// the end of the
															// text
						else
							edittext.setSelection(offset - 1);
	...
	// /**
	// * Shows a simple dialog containing a list from the itemsId resource. The
	// * onClickListener is set to this, see onClick(...) below
	// */
	...
	// /**
	// * @param popupCode
	// * Code indicating which popup is currently opened for the
	// * onClick getter
	// */
	// private int popupCode;

	@Override
	public void onClick(DialogInterface dialog, int which) { // insert one of
																// the
																// additional
																// popups to the
																// current Focus
																// field
		Log.i("CustomKeyboard", "onClick");
		String insertion = "";
		// switch(popupCode){
		// case 55009:
		// insertion =
		// mHostActivity.getResources().getStringArray(R.array.insert_const_values)[which];
		// break;
		// case 55010:
		// insertion =
		// mHostActivity.getResources().getStringArray(R.array.insert_log_values)[which];
		// break;
		// case 55011:
		// insertion =
		// mHostActivity.getResources().getStringArray(R.array.insert_conv_values)[which];
		// break;
		// case 55012:
		// insertion =
		// mHostActivity.getResources().getStringArray(R.array.insert_trig_values)[which];
		// break;
		// }
		View focusCurrent = window.getCurrentFocus();
	 ...
	 * 
	 * @param goEnabled
	 *            true if you want haptic feedback, falso otherwise
	 */
